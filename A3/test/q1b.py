...
def decode(model, lits, n):
    ...
    for order in range(n * n):
        for i in range(n):
            for j in range(n):
                if model.evaluate(lits[i][j][order]):
                    moves.append([i, j])
    return moves
...
def encoding(n):
    ...
    and_clauses = [lits[0][0][0], lits[n - 1][n - 1][n * n - 1]]
    and_clauses += [Or(lits[0][1][1], lits[1][0][1])]
    and_clauses += [Or(lits[n - 1][n - 2][n * n - 2], lits[n - 2][n - 1][n * n - 2])]
    for i in range(n):
        for j in range(n):
            if (i, j) != (0, 0) and (i, j) != (n - 1, n - 1):
                adj = valid(neighbours(i ,j), n)
                for order in range(1, n * n - 1):
                    next_clauses = []
                    prev_clauses = []
                    for cell in adj:
