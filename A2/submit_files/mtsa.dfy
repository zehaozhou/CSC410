...
{   
    assert forall i, j, a1 :: 0 <= i <= |a| && 0 <= j <= |a| && j < i && a1 == a[j..i] ==> a[i - 1] == a1[|a1| - 1] && a[j..i - 1] == a1[..|a1| - 1];
}
    ...
    ensures exists i :: 0 <= i <= a.Length  && sum(a[i..]) == mts // postcondition for part (b)
{
   ...
   assert sum(a[i..i]) == mts;
        ...
        invariant exists k :: (0 <= k <= i < a.Length && sum(a[k..i]) == mts) || (0 <= k <= i == a.Length && sum(a[k..]) == mts)
    {
        ...
        assert sum(a[i..i]) == 0;
        assert condition_sum <= 0 ==> sum(a[i..i]) == 0 == mts;
        assert condition_sum > 0 ==> sum(a[k..i]) == sum(a[k..i - 1]) + a[i - 1] == (mts - a[i - 1]) + a[i - 1] == mts;
    }
}